...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (monitorContainer) {
        ...
        let kpisDataAtual = []; // Variável para guardar os dados da tabela
        let kpiDataParaExportar = { ano: new Date().getFullYear(), mes: new Date().getMonth() }; // Para o nome do arquivo
        // --- FIM DA ADIÇÃO ---
        ...
        function inicializarMonitorPage() {
            ...
            renderizarCalendarioPreventivas(dataAtual.getFullYear(), dataAtual.getMonth());
            renderizarKPIsMensais(dataAtual.getFullYear(), dataAtual.getMonth());
            const btnExportar = document.getElementById("btn-exportar-kpi");
            if (btnExportar) {
                btnExportar.addEventListener('click', () => {
                    // Chama a função de exportação com os dados guardados
                    exportarKPIsParaCSV(kpisDataAtual, kpiDataParaExportar);
                });
            }
        }
        function exportarKPIsParaCSV(kpis, data) {
            if (kpis.length === 0) {
                alert("Não há dados para exportar.");
                return;
            }

            // Nomes dos meses para o arquivo
            const nomesMeses = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"];
            const nomeArquivo = `KPIs_Manutencao_${nomesMeses[data.mes]}_${data.ano}.csv`;

            // 1. Criar o Cabeçalho (Header) do CSV
            // Usamos \uFEFF para garantir a codificação correta (UTF-8 BOM) para o Excel
            let csvContent = "data:text/csv;charset=utf-8,\uFEFF";
            csvContent += "Maquina,Disponibilidade (%),MTTR (Horas),MTBF (Horas),N de Falhas,Downtime (Horas)\r\n";

            // 2. Adicionar as Linhas (Rows)
            kpis.forEach(m => {
                // Coloca a tag entre aspas para tratar nomes com vírgula ou espaços
                const tagFormatada = `"${m.tag}"`;
                let row = [
                    tagFormatada,
                    m.disponibilidade,
                    m.mttr,
                    m.mtbf,
                    m.num_falhas,
                    m.total_downtime_horas
                ].join(","); // Separa as colunas por vírgula
                csvContent += row + "\r\n"; // Adiciona uma quebra de linha
            });

            // 3. Criar o link e simular o clique para download
        ...
        // --- NOVA FUNÇÃO PARA O CALENDÁRIO ---
        async function renderizarCalendarioPreventivas(ano, mes) {
            const container = document.getElementById('calendario-preventivas-container');
            if (!container) return;
            container.innerHTML = "<p>Carregando calendário...</p>";

            // Nomes dos meses (JS mes é 0-11)
            const nomesMeses = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
            const nomeMes = nomesMeses[mes];

            // 1. Buscar os dados das preventivas
            let eventosPorDia = {};
            try {
                // O backend espera o mês 0-11 (como no JS)
                const response = await fetchSeguro(`${API_URL}/stats/preventivas-mes?ano=${ano}&mes=${mes}`, { method: 'GET' });
                if (!response.ok) throw new Error("Falha ao buscar preventivas");
                const preventivas = await response.json();

                // Organiza as preventivas por dia
                preventivas.forEach(p => {
                    const dia = p.dia; // O backend já nos deu o dia (1-31)
                ...
                if (error.message !== "Sessão inválida") {
                    ...
                    container.innerHTML = "<p class='mensagem erro'>Erro ao carregar dados do calendário.</p>";
                    return;
                }
            }

            // 2. Lógica para gerar o calendário
            let primeiroDiaDoMes = new Date(ano, mes, 1).getDay(); // 0=Domingo, 1=Segunda...
            let diasNoMes = new Date(ano, mes + 1, 0).getDate(); // Pega o último dia do mês atual

            // 3. Montar o HTML
            let html = `<div class="calendario-header">
                            <button id="btn-prev-mes">‹ Anterior</button>
                            <h3>${nomeMes} ${ano}</h3>
                            <button id="btn-next-mes">Próximo ›</button>
                        </div>`;

            html += `<table class="calendario-tabela">
                        <thead>
            ...
            for (let i = 0; i < 6; i++) { // Máximo de 6 semanas
                ...
                for (let j = 0; j < 7; j++) { // 7 dias da semana
                    if (i === 0 && j < primeiroDiaDoMes) {
                        ...
                        htmlSemana += '<td class="dia-vazio"></td>';
                        ...
                        if (diaAtual === dataHoje.getDate() && mes === dataHoje.getMonth() && ano === dataHoje.getFullYear()) {
                            classeHoje = " dia-hoje";
                        }

                        // Adiciona os eventos
                        let eventosHtml = "";
                        if (eventosPorDia[diaAtual]) {
                            eventosPorDia[diaAtual].forEach(evento => {
                                // Cria um link para a OS
                                eventosHtml += `<a href="ordem.html?id=${evento.id}" class="evento-preventiva" target="_blank" title="${evento.descricao}">
                                                    <strong>${evento.equipamento_tag || 'Equip.'}</strong> (OS #${evento.id})
                                                </a>`;
                            });
                        }

                        htmlSemana += `<td class="dia-calendario${classeHoje}">
                                        <span class="calendario-dia-numero">${diaAtual}</span>
                                        <div class="eventos-container">${eventosHtml}</div>
                                       </td>`;
                        diaAtual++;
                    }
                }
                htmlSemana += "</tr>";
                html += htmlSemana;
                if (diaAtual > diasNoMes) break; // Para de desenhar semanas se já acabou o mês
            }

            html += `</tbody></table>`;
            container.innerHTML = html;

            // 4. Adicionar listeners nos botões de navegação
            document.getElementById('btn-prev-mes').addEventListener('click', () => {
                let novaData = new Date(ano, mes, 1);
                novaData.setMonth(novaData.getMonth() - 1);
                renderizarCalendarioPreventivas(novaData.getFullYear(), novaData.getMonth());
            });
        ...
        // --- NOVA FUNÇÃO PARA OS KPIS (MTTR/MTBF) ---
        async function renderizarKPIsMensais(ano, mes) {
            const container = document.getElementById('kpi-maquinas-container');
            const tituloMes = document.getElementById('kpi-titulo-mes');
            if (!container || !tituloMes) return;

            container.innerHTML = "<p>Carregando KPIs...</p>";

            const nomesMeses = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
            tituloMes.textContent = `Indicadores de Manutenção (${nomesMeses[mes]}/${ano})`;

            try {
                const response = await fetchSeguro(`${API_URL}/stats/kpis-maquina-mes?ano=${ano}&mes=${mes}`, { method: 'GET' });
                if (!response.ok) throw new Error("Falha ao buscar KPIs (Erro de Rede)"); // Erro de rede/http
                
                const kpis = await response.json();
                
                // --- MUDANÇA IMPORTANTE ---
                // Verifica se o backend nos enviou uma mensagem de erro (que agora é um JSON válido)
                if (kpis.error) {
                    console.error("Erro vindo do backend:", kpis.error);
                    // Exibe o erro exato do Python na tela
                    container.innerHTML = `<p class='mensagem erro'>${kpis.error}</p>`;
                    kpisDataAtual = []; // Limpa dados
                    return;
                }
                // --- FIM DA MUDANÇA ---
                ...
                kpisDataAtual = kpis; 
                kpiDataParaExportar = { ano: ano, mes: mes };

                if (kpis.length === 0) {
                    container.innerHTML = "<p>Nenhum dado de KPI encontrado para este período.</p>";
                    kpisDataAtual = []; 
                    return;
                }

                // Montar tabela de KPIs
                let tabelaHtml = `<table class="tabela-kpi">
                                <thead>
                                    <tr>
                                        <th>Máquina (TAG)</th>
                ...
                kpis.forEach(m => {
                    tabelaHtml += `
                        <tr>
                            ...
                            <td class="kpi-disponibilidade">${m.disponibilidade} %</td>
                            <td class="kpi-mttr">${m.mttr} h</td>
                            <td class="kpi-mtbf">${m.mtbf} h</td>
                            <td>${m.num_falhas}</td>
                            <td>${m.total_downtime_horas} h</td>
                        </tr>
                    `;
                });
                ...
                container.innerHTML = tabelaHtml;
                 ...
                 // Este catch agora só pega erros de rede ou se o JSON estiver *totalmente* quebrado
                 if (error.message !== "Sessão inválida") {
                    console.error("Erro ao carregar KPIs (Catch do Frontend):", error);
                    container.innerHTML = "<p class='mensagem erro'>Erro ao carregar KPIs (Falha de comunicação).</p>";
                 }
            }

            // Adicionar listeners nos botões DESTE COMPONENTE
            // Usamos .onclick para garantir que estamos substituindo o listener antigo
            document.getElementById('btn-prev-mes-kpi').onclick = () => {
                let novaData = new Date(ano, mes, 1);
                novaData.setMonth(novaData.getMonth() - 1);
                renderizarKPIsMensais(novaData.getFullYear(), novaData.getMonth());
            };
            
            document.getElementById('btn-next-mes-kpi').onclick = () => {
                let novaData = new Date(ano, mes, 1);
                novaData.setMonth(novaData.getMonth() + 1);
                renderizarKPIsMensais(novaData.getFullYear(), novaData.getMonth());
            };
